...
Note that this is intentionally ignoring domain-connected machines and their
attack surfaces.
...
a Windows service or scheduled task) access. This is usually because everything
under the two `C:\Program Files` directories are not writable by standard users.
...
This is usually not in scope for most bounty programs. Also, TLS certificates do
not usually allow us to do it. But, if you can mess with the certificate
checks then you might have a case.

#### Nintendo
See this report in Nintendo's bug bounty program.

* https://hackerone.com/reports/894922

The signature verification of 3DS is bypassed. This allows an attacker to
successfully sit between the console and the internet.
...
The application used a statically linked `libcurl` library that disabled
certificate checks if the URL had some strings in it like
`api/clientversion.xml`. The attacker could MitM the connection and bypass the
certificate check because the update URL had the string above. Then, they could
send their own tainted binary as the updater to be executed as SYSTEM.
...
and is still open (no bounty awarded, either). A Java application used
[getdown][getdown] to download its update over HTTP. With HTTP, I could MitM the
...
because the digest file was also downloaded over HTTP. So, an attacker could
replace it, too.

I created a fake Python HTTP server to act as my spoofed server. I created my
own digest and backdoored jar. The jar was a Swing application that only
...
access and then execution (usually by a Windows service).
...
binary and then wait for the updater service to run it as SYSTEM.
...
users from modifying it. The binary is executed if the check passes.
...
condition to replace the binary after the check but before it is executed.
...
binary at `%ProgramData%\Backblaze\bzdata\bzupdates` if it did not exist. The
DACL of the directory did not give standard users write access. But, the updater
did not check if the directory was already created and would store the updater
there without changing its permissions.

Hence, a local attacker could create the directory and then replace the
downloaded updater before execution. The bug did not need it but in these cases,
we are racing against the updater. The way to win these races is often through
OpLocks. This is usually pretty easy with the [SetOpLock][setoplock] utility
in the great [symboliclink-testing-tools][symboliclink-testing-tools] suite by
...
still open with no bounty, again (see a pattern?). It's a bug in the same
application as my first undisclosed bug. I discovered that the installer changed
the DACL of the entire application directory under `C:\Program Files`. The new
DACL allowed standard users to write to that directory.
...
Window service or UAC prompts. Usually, applications that need to do this are
stored under `%APPDATA%` (e.g., VS Code).

In this bug, I modified the uninstaller (which always ran elevated). A local
attacker could swap the binary with a backdoored executable and then wait for or
convince an admin to run the uninstaller elevated.
...
and then reported it recently. It's still in review and I doubt it will get
disclosed.

## Named Pipe
The application used a Windows service to do updates. The service talked to the
userland application using a [named pipe][namedpipe]. Named pipe is another
great attack surface. I want to write a similar blog post about them later but
for now you can read these great series of posts by
...
The named pipe had a (fortunately) json-based protocol. The pipe was named
`george` (lol).

{{< imgcap title="George" src="03-george.png" >}}

Everyone could open a pipe and talk to the updater service. This is not a
...
then run it. Unfortunately, we could only tell the service to do these without
any specifics. E.g., we could not specify where to download the file or which
file to execute.
...
c:\ProgramData\[redacted]\Updates
  RW BUILTIN\Users   <---- WE HAVE RW ACCESS HERE
  RW NT AUTHORITY\SYSTEM
  RW BUILTIN\Administrators
```

By now, you are thinking we can use Jason's second bug and overwrite the
downloaded file. This is what I did but the file was not executed by the
installer.

Looking at procmon events, I located the events where the file is accessed.
...
After clicking on one of the events above I got this stack.

{{< imgcap title="Call stack in Procmon" src="07-procmon-stack.png" >}}

Note the `WinVerifyTrust` calls. These are usually used to check the signature
of a file. My first try was swapping the file with a signed binary like
`taskmgr.exe` (which has a valid signature). But, it not work. Seemed like the
updater was looking for a specific signature and not just any valid one.
...
analysis, I landed at the following location. We can see two calls to
`WinVerifyTrust` and a call to `sub_100550`. After more dynamic analysis, I
realized the sub checks the Common Name (CN) of the certificate against
...
## Enter Electron
A typical Electron app on Windows comes with a bunch of executables. We cannot
...
The executable is `Discord.exe` which is signed by `Discord Inc.`. In my
...
The asar container is unsigned. To be more clear, it's just a bunch of data and
the format does not support signing. This means we can modify it and the signed
binary `Discord.exe` will run our modified code.

I created a backdoored Electron application:
...
1. The application's updater service downloads the update and stores it in `C:\ProgramData\[redacted]\GUID.exe`.
...
5. Replace the `resources/app.asar` file in the destination with my own backdoored app.
